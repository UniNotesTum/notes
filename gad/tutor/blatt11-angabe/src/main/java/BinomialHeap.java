...
public class BinomialHeap<T extends Comparable<T>> {
  ...
  /**
   * Diese Methode addiert zwei binomiale Bäume und
   * einen Carry-Baum zusammen. Jeder der drei Bäume
   ...
   */
  private Pair<Optional<BinomialTreeNode<T>>, Optional<BinomialTreeNode<T>>> add(Optional<BinomialTreeNode<T>> a, Optional<BinomialTreeNode<T>> b, Optional<BinomialTreeNode<T>> carry) {
    @SuppressWarnings("unchecked")
    Optional<BinomialTreeNode<T>>[] nodes = (Optional<BinomialTreeNode<T>>[]) Array.newInstance(Optional.class, 3);
    ...
    if(count <= 1)
      ...
      return new Pair<>(nodes[2], Optional.of(BinomialTreeNode.merge(nodes[0].get(), nodes[1].get())));
    }
  }
  
  /**
   * Diese Methode wird aufgerufen, wenn sich ein Baum durch die
   * decreaseKey()-Operation verändert hat.
   * 
...
//      throw new RuntimeException("Invalid tree!");
  ...
  public void decreaseKey(Object handle, T elementNew) {
    @SuppressWarnings("unchecked")
    BinomialHeapHandle<T> handleCasted = (BinomialHeapHandle<T>)handle;
    handleCasted.getNode().decreaseKey(elementNew);
//    if(!validHeap())
//      throw new RuntimeException("Invalid tree!");
  }
}
